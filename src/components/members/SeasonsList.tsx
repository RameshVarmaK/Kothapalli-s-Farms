  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4.5 border-b border-slate-200 bg-slate-50/50">
        <h3 className="font-bold text-xs uppercase tracking-widest text-slate-400">{t('Sown Cropping Cycles')}</h3>
      </div>

      {seasons.length === 0 ? (
        <EmptyState
          icon={<Grid size={22} />}
          title={t('No crop seasons yet')}
          description={
            fields.length === 0
              ? t('Register a field first, then sow a crop season on it to start logging expenses, labour and harvest against it.')
              : t('A season tracks one crop cycle on one field, from sowing to harvest. Sow your first one to start logging activity against it.')
          }
          ctaLabel={fields.length === 0 ? t('Register a Field First') : t('Sow First Crop')}
          onCta={() => {
            if (fields.length === 0) {
              onGoToFields();
            } else {
      ...
      <table className="w-full text-left text-xs border-collapse">
        <thead>
          <tr className="border-b border-slate-200 text-slate-400 bg-slate-50/50 font-bold text-[10px] uppercase tracking-widest">
            <th className="px-6 py-4">{t('Sown Field')}</th>
            <th className="px-6 py-4">{t('Crop Name')}</th>
            <th className="px-6 py-4">{t('Sown Date')}</th>
            <th className="px-6 py-4 text-right">{t('Actions / Status')}</th>
          </tr>
        </thead>
          ...
          {seasons.map(s => {
            ...
            return (
              <tr key={s.id} className="hover:bg-slate-50/50 font-medium pb-2">
                <td className="px-6 py-4 font-bold text-slate-800">{f ? f.name : t('Unknown')}</td>
