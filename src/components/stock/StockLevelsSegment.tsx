  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4.5 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
        <h3 className="font-bold text-xs uppercase tracking-widest text-slate-400">{t('Stock Reserves Valuation')}</h3>
        <button
      ...
      <table className="w-full text-left text-xs border-collapse">
        <thead>
          <tr className="border-b border-slate-200 text-slate-400 bg-slate-50/50 text-[10px] uppercase tracking-widest font-bold">
            <th className="px-6 py-4">{t('Material Identifier')}</th>
            <th className="px-6 py-4">{t('Category')}</th>
            <th className="px-6 py-4 text-right">{t('Available Qty')}</th>
            <th className="px-6 py-4 text-right">{t('Weighted Avg Cost')}</th>
            <th className="px-6 py-4 text-right">{t('Reserve Value')}</th>
            ...
            <tr>
              <td colSpan={6} className="px-6 py-0">
                <div className="flex flex-col items-center justify-center text-center py-12">
                  ...
                  <h4 className="font-bold text-slate-800 text-sm mb-1.5">{t('No farm inputs registered')}</h4>
                  <p className="text-xs text-slate-400 max-w-xs leading-relaxed mb-5">
                    {t('Register seed, fertilizer, pesticide, or fuel types here first — then log purchases and field usage against them.')}
                  </p>
                  <button
                    onClick={onCreateItemType}
