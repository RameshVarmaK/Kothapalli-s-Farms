        ...
        {purchases.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-16 px-6">
            ...
            <h4 className="font-bold text-slate-800 text-sm mb-1.5">
              {stockItems.length === 0 ? t('Register an input type first') : t('No purchases logged yet')}
            </h4>
            <p className="text-xs text-slate-400 max-w-xs leading-relaxed mb-5">
              {stockItems.length === 0
                ? t('You need at least one input type (seed, fertilizer, etc.) before you can log a purchase against it.')
                : t('Log every bag of fertilizer, seed, or fuel bought — it builds the weighted-average cost used across field usage.')}
            </p>
            <button
            ...
            >
              <Plus size={14} />
              {stockItems.length === 0 ? t('Create First Input Type') : t('Log First Purchase')}
            </button>
          </div>
        ) : (
          [...purchases].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(p => {
            const item = stockItems.find(i => i.id === p.stockItemId);
            const payer = members.find(m => m.id === p.paidByMemberId)?.name || t('Unknown');
              ...
              <div key={p.id} className="p-4.5 flex justify-between items-center text-xs hover:bg-slate-50/30">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1.5 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                    ...
                    <span>•</span>
                    <span>{t('Paid by')} {payer}</span>
                    <span>•</span>
                    <span className="font-mono text-slate-500">{t('Unit Cost:')} {currency}{Math.round(calculatedRate).toLocaleString('en-IN')}</span>
                  </div>
                </div>
